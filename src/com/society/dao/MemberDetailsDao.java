	...
	public List getAllMembers()
	{
		...
		public List<GetMemberDetailsBean> getMemberDetail(String fkMemberid, String memberName) 
		{
			...
			try {
				hbu = HibernateUtility.getInstance();
				session = hbu.getHibernateSession();
				//Query query2=session.createSQLQuery("select * FROM member_details WHERE pk_member_id='"+fkMemberid+"' AND concat (first_name,' ',last_name) ='"+memberName+"'" AND postion="LIKE '%m");
		...
		try{	
			...
			Query query=session.createQuery("From MemberDetailsHibernate");
			//Query query=session.createQuery("select * From MemberDetailsHibernate where  postion=membername");
