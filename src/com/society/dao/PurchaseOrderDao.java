		...
		//get Client Records
			...
			{				
				...
				try
				{
					hbu = HibernateUtility.getInstance();
					session = hbu.getHibernateSession();
					Query query = session.createSQLQuery("select poc.pk_vendor_id, poc.billNo from purchase_order_create poc where poc.fkVendorId = "+fkVendorId+" GROUP BY poc.billNo");
					list = query.list();
